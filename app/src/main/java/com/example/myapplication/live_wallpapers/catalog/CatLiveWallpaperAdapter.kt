...
class CatLiveWallpaperAdapter(
    private val context: Context,
    private val wallpaperList: List<CatLiveWallpaperItem>,
    private val navController: NavController
) : RecyclerView.Adapter<CatLiveWallpaperAdapter.MyHolder>() {
    ...
    override fun onBindViewHolder(holder: MyHolder, position: Int) {
        ...
        Glide.with(context).load(item.imageUrl.toInt())
            .placeholder(R.drawable.wallicon)
            .error(R.drawable.baseline_error_outline_24).into(holder.wallpaperImage)

        holder.itemView.setOnClickListener {
            val intent = Intent(context, CatLiveWallpaperDetailActivity::class.java).apply {
                putExtra(CatLiveWallpaperDetailActivity.EXTRA_WALLPAPER_TITLE, item.title)
            }
